...
\midrule
Purpose & General-purpose secure file storage & Snapshot encryption for Phantom OS \\[4pt]
User interface & GUI via ui\_config / ui\_report ROM & None (headless) \\[4pt]
Authentication & Passphrase entered at runtime & Key file read from USB drive \\[4pt]
File system driver & rump\_vfs (ext2) & lwext4 (ext4) \\[4pt]
fsck support & No & Yes (configurable via \texttt{fsck\_apply}) \\[4pt]
Storage backend & Image file via File\_system session & Direct block device session \\[4pt]
Trust Anchor storage & File\_system session & USB block session \\[4pt]
Online extend / rekey & Yes & Removed \\[4pt]
Image file creation & Pre-allocated via truncate\_file & Skipped \\[4pt]
Block size & 512 & Standardized to 4~KB \\[4pt]
