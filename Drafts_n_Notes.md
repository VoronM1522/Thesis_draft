...
- **ru**
  ...
  ```
  Эволюция операционных систем на протяжении десятилетий определялась доказавшей свою жизнеспособность концептуальной моделью, сформировавшейся в середине XX века с появлением UNIX. Однако сегодня она всё чаще демонстрирует фундаментальные ограничения. Попытки обойти их еще в конце XX века привели в том числе и к появлению концепции операционных систем с персистентной памятью. Несмотря на достоинства такой парадигмы исследования фактически были прекращены, так как столкнулись с аппаратными ограничениями. С тех пор производительность компьютеров сильно выросла, что дало толчок новым работам в этом направлении. Яркой иллюстрацией этого является ОС Phantom. Однако написание и внедрение полностью новой операционной системы весьма длительно, трудоемко и влечет за собой большое количество ошибок. Смягчить эти последствия помогает использование уже отработанных механизмов, чему способствует портирование на Genode. В данной работе мы рассматриваем процесс адаптации компонентов для PhantomOS на Genode. Мы описываем трудности, с которыми приходится сталкиваться, недостатки ОС и рассказываем о решениях, которые помогают избавиться от этих проблем или смягчить их. Все еще актуальным остается вопрос безопасного хранения снапшотов. Для его решения был адаптирован file\_vault, выполняющий шифрование на блочном уровне и предоставляющий сессию файловой системы. В результате появился se\_vault. Но обратной стороной безопасности, ограничивающей возможность его использования, стало существенное снижение производительности: примерно в 25 раз, что обусловленно использованием эксперементальной библиотеки Tresor. Полученные результаты открывают направление для дальнейших исследований в области безопасности персистентных операционных систем и могут способствовать их более широкому применению.
  ```
...
- **ru**
  ...
  Научная новизна заключается в недостаточном уровне ис следования темы и отсутствии каких-либо готовых механизмов решения выявленной проблемы.
  Работа организована следующим образом: Глава 2 содержит обзор литературы по ортогональной персистентности, особенностям PhantomOS и методам защиты данных в состоянии хранения. Глава 3 описывает методологию: модель сценариев использования, требования к компоненту и план реализации. Глава 4 посвящена адаптации компонента file\_vault и сопутствующим адаптациям смежных компонентов. Глава 5 анализирует результаты, ограничения и направления дальнейших работ. заключение содержитcя в Главе 6.
...
- **ru**
  ...
  В данной работе мы будем заниматься внедрением среды для безопасного хранения снапшотов. Это будет хранилище с шифрованием на блосном уровне, что повысит безопасность в сравнении с шифрованием на уровне файловой системы и выше, скрыв метаинформацию и усложнив процесс получения данных о системе. Этому также способствует то, что в Genode уже существует библиотека \cite{GitHub_tresor} для этого и тестовый компонент \cite{GitHub_file_vault, Page_file_vault}. 
  
  Tresor выполняет AES шифрование для блоков размером 4K. Ключ виртуального блочного устройства шифруется мастер-ключом, за который отвечает Trust Anchor. При записи используется CoW, что обеспечивает атамарность операции и благоприятно сказывается на возможности восстановления после сбоев. Для контроля целостности данных компонент использует SHA-256.
  ```
...
В данной работе мы будем заниматься внедрением среды для безопасного хранения снапшотов. Это будет хранилище с шифрованием на блосном уровне, что повысит безопасность в сравнении с шифрованием на уровне файловой системы и выше, скрыв метаинформацию и усложнив процесс получения данных о системе. Этому также способствует то, что в Genode уже существует библиотека \cite{GitHub_tresor} для этого и тестовый компонент \cite{GitHub\_file\_vault, Page\_file\_vault}. 

Tresor выполняет AES шифрование для блоков размером 4K. Ключ виртуального блочного устройства шифруется мастер-ключом, за который отвечает Trust Anchor. При записи используется CoW, что обеспечивает атамарность операции и благоприятно сказывается на возможности восстановления после сбоев. Для контроля целостности данных компонент использует SHA-256.



Библиотека Tresor реализует зашифрованное виртуальное блочное устройство.

```latex
\textit{Структура хранилища.} Физическое пространство блочного устройства
разделено на несколько логических областей. Суперблок (superblock) хранит
корневые метаданные всего хранилища: ссылки на корни деревьев, текущее
поколение (generation), состояние выполняемых операций и зашифрованные
ключи шифрования блоков. На физическом устройстве хранится несколько
слотов суперблока, что обеспечивает атомарность обновления состояния.
Виртуальное блочное устройство (Virtual Block Device, VBD) организовано
в виде дерева: внутренние узлы содержат хеши дочерних узлов, листья ---
зашифрованные блоки данных по 4~КБ. Свободное дерево (Free Tree) управляет
аллокацией физических блоков с помощью аналогичной древовидной структуры.

\textit{Конфиденциальность.} Блочные данные шифруются по алгоритму AES.
Ключ шифрования блоков хранится в зашифрованном виде внутри хранилища.
Для его шифрования используется мастер-ключ, хранящийся в отдельном
компоненте Trust Anchor. Мастер-ключ зашифрован с применением алгоритма
AES Key Wrap (RFC~3394) на основе 256-битного SHA-хеша пользовательской
парольной фразы. Таким образом, физический доступ к носителю без знания
парольной фразы не позволяет расшифровать данные.

\textit{Целостность.} Каждый блок данных в VBD сопровождается 256-битным
SHA-хешем. Хеши организованы в дерево Меркла: каждый внутренний узел
содержит хеши своих дочерних узлов. Корневой хеш всего дерева хранится
в Trust Anchor и верифицируется при каждом обращении к хранилищу.
Это позволяет обнаружить как случайное повреждение данных, так и
намеренную их модификацию.

\textit{Консистентность и поколения.} Tresor использует модель поколений
(generations): каждая операция записи порождает новое поколение, не
перезаписывая старые блоки напрямую. Вместо этого применяется
копирование при записи (CoW): модифицированные блоки записываются
в новые физические адреса, внутренние узлы дерева обновляются снизу
вверх. Суперблок обновляется только после успешной записи всего нового
состояния, а в Trust Anchor сохраняется хеш нового суперблока. До этого
момента предыдущее состояние остаётся полностью валидным. Внезапное
завершение процесса или потеря питания в любой момент оставляют
хранилище в последнем консистентном состоянии.

\textit{Снапшоты.} Механизм поколений позволяет Tresor хранить несколько
снапшотов одновременно. Снапшот --- это зафиксированное поколение VBD,
на блоки которого сохраняются ссылки из суперблока. Блоки, разделяемые
между поколениями, не дублируются, что обеспечивает экономию места.
Снапшот освобождается операцией Discard, после чего его физические
блоки возвращаются в Free Tree.

\textit{Перешифрование (rekeying).} Tresor поддерживает онлайн-перешифрование:
замену ключа шифрования блоков без остановки устройства \cite{CBE_rekeying}.
При этом все блоки всех хранимых поколений последовательно расшифровываются
старым ключом и зашифровываются новым. Операция разбита на атомарные
шаги, между которыми устройство остаётся доступным для чтения и записи.
Прогресс перешифрования фиксируется в суперблоке, что позволяет
возобновить его после перезагрузки.
```
...
- **ru**
  ...
  В этой работе у нас нет цели создания универсального инструмента или сохранения всего функционала искомого, поэтому в процессе адаптации будут оставлены и добавлены только необходимые функции. Измененный компонент далее обозначается se\_vault.
  
  File\_vault управляет конфигурацией компонентов, отвечающих за шифрование.  Это оркестратор состояний в sandbox, предоставляющий клиентам File\_system сессию защищенного хранилища, единственным клиентом которой в нашем случае будет snapper. Стартуя, он запрашивает пароль у пользователя. При совпадении определяет состояние хранилища и либо инициализирует его, либо разблокирует. Перед завершением работы он запечатывает компонент, что приводит к сбросу всех блоков и гарантирует консистентное состояние хранилища. Сам file\_vault получает и маршрутизирует к дочерним (оркестрируемым) компонентам сессии файловых систем, в которых находятся файл-образ защищенного хранилища и информация, принадлежащая Trust Anchor. Инициализация происходит в автоматическом режиме. Может быть запрошено задание парольной фразы, но это поведение конфигурируется. 
  
  UI был от оценен, как избыточный, поскольку snapper использует только программный интерфейс. Через UI можно также ввести парольную фразу или запросить изменение размера ФС. В нашем случае для аутентификации используется флешка с ключом (и другими данными Trust Anchor), а изменение размера ФС в процессе работы не предусматривается, поэтому от UI можно полностью отказаться. Это привело к значительному уменьшению объема кода и количества обрабатываемых состояний. Помимо этого был удален весь код для генерации fs_query компонентов, поскольку для корректного перехода между состояниями оказалось достаточно мониторинга кодов возврата компонентов.
  
  Далее была добавлена опция проверки и восстановления состояния ФС с помощью fsck. Изначально file\_vault создавал файловую систему ext2. Она была заменена на ext3, а позже - на ext4. Причиной тому послужило отсутствие журналирования, из-за чего момогут появиться проблемы при восстановлении после сбоев. Также внимание привлек компонент rump_fs, у которого нет возможности конфигурировать размер кеша, что понадобится для достижения баланса надежности и скорости. Он был заменен на lwext4.
  ...
  Аналогично проверке функциональной части тестировались штатное заершение и опциональное восстановление с fsck. Контролировалось, что при нажатии на кнопку выключения делается снимок, звершается isomem, затем - snapper, размонтируется файловая система и завершается vfs, безопасное хранилище блокируется и se\_vault завершается. Все этапы были пройдены без нарушений. После этого проверялось, что при новом запуске флаг монтирования файловой системы не выставлен, что тоже было достигнуто. Для проверки восстановлени с fsck в конфигурации компонента выставлялась добавленная опция \textit{fsck\_apply="no"}. После этого система запускалась и завершалась аварийно. Тест считался пройденым, если при повторном старте запускался fsck, устранял ошибки, и система работала штатно. После изменения размеров блоков fsck сигнализироал о успешном исправлени ошибок кодом "1", однако при дальнейшей работе появлялись ошибки lwext4, связанные с \textit{bcache}. Проблему удалось решить применением опции \textit{"-B 4096"}. После этого проблем не наблюдалось.
  
  Напоследок было проведено тестирование скорости. Планоровалось замерить и сравнить для шифрованного и обычного вариантов время создания каждого из первых трех снапшотов, а также скорость восстановления при загрузке. Приемлимыми считались результаты, если быстродействие сокращалось не более чем в 2 раза, что является очень большими издержками при шифровани. К сожалению, первый же результат поразил, показав на первом снапшоте более, чем в сотню раз, возросшее время. Это при том, что первый снимок выполняется быстрее всего, ведь не приходится удалять предыдущие. На последующих разрыв возрастал еще больше. От части проблема была в стандартных опциях QEMU, для pc, в которых используется \textit{-cpu Nehalem-v2}. Из-за этого не работало аппаратное ускорение AES и SHA. После исправления на \textit{-cpu host} быстродействие значительно возрасло, но все равно оставалось очень низким. Были предприняты попытки замены ahci драйвера на nvme, что незначительно повысило быстродействие. Тестировались варианты без флешки (вместо нее использовался образ с доступом через nvme), с уменьшенным размером дерева в tresor, но и это не дало требуемого прироста. Конечный результат следующий: первый снапшот создается без шифрования ~2 секунды, с шифрованием ~51 секунду. Позже выяснилось, что проблема быстродействия tresor известна и пока не была решена. Она заключается в самой реализации библиотеки, которая не оптимизирована для многопоточной работы. Некоторых значимыхуспехов удалось достичь лиши в контексте инициализации в сравнении с оригинальным file\_vault, поскольку нынешняя реализация. Ранее создание объемного файла-образа занимало много времени. Так, образ для пользователььской ФС на 4 Gb создавался около 20 минут. Нынешняя реализация пропускает этот этап вовсе.
  ```
...
После замены rump_fs на lwext4 возникли проблемы с cозданием файла нужного размера. Оказалось, что  \textit{truncate_file} не проверяет размер файла после его создания, а lwext4 создает файл меньшего размера. Решить проблему удалось, добавив перемещение указателя в функции \textit{ftruncate} lwext4.

Далее система стала запускаться, но при повторном запуске падала, выдавая множество несовпадения ошибок хеша. Эксперементы по уменьшению размера кеша или его отключению результатов не дали. Позже проблема была обнаружена и устранена. Дело в том, что tresor обновляет хеш во время вызова  \textit{sync}. При вызове  \textit{sync} lwext4 просто сбрасывает содержимое буферов на диск. Был добавлен блок кода, передающий вызов \textit{sync} на нижние уровни vfs, что позволило tresor корректно получать этот вызов и обновлять хеш состояния своевременно.

На момент завершения вышеописанных работ в системе все еще не было способа штатного завершения работы, из-за чего tresor никогда не сбрасывал все блоки явно, и при неудачном завершении появлялся риск потери последнего поколения. Устранено это было внесением изменений в isomem, snapper и vfs. В isomem был добавлен callback для кнопки выключения. После нажатия на кнопку система выставляет флаг, сигнализируя о готовности к выключению. Это сразу (во время ожидания снапшота) или после завершения текущего снимка запускает создание последнего снапшота, после чего isomem высвобождает ресурсы CPU, но никак не сигнализирует системе о своем завершении. Для исправления этого процесс завершения был заменен вызовом  \textit{env.parent().exit(0)}. В snapper и vfs были добавлены счетчики клиентов, которые приводили к завершению тем же способом при его обнулении, но для vfs еще был добавлен влаг конфигурации  для выхода, так как в процессе переключения между состояниями этот счетчик обнулялся и происходило преждевременное завершение работы vfs. Помимо этого процесс завершения работы vfs был модифицирован таким образом, чтобы все подключенные плагины корректно завершали свою работу. Это понадобилось для выполнения размонтирования lwext4 и выставления соответствующего флага ФС. В противном случае при использовании fsck следующий запуск системы начинается с проверки. Счетчик также был добавлен в se\_vault, но внем после отключения всех клиентов происходило запечатывание хранилища и выставление флага для дочернего vfs. Только после этого se\_vault завершает работу.
...
В этой главе мы более глубоко окунемся в результаты проделанной работы. Будут рассмотрены результаты, ограничения работы, недостатки и способы их решения. В конце каснемся темы будующих работ в этом направлении.

\section{Results}

В процессе исследования компонент file\_vault был адаптиован для использования snapper'ом в качестве хранилища снапшотов. Интеграция прошла прозрачно для snapper и других компонентов, потребовались небольшие изменения для корректного завершения работы, которые были нужны и без нашего компонента. Лишь некоторые изменения lwext4 можно считать действительно необходимыми только для данной работы. Сравнительный анализ получившегося и искомого компонентов можно видеть на Table~\ref{tab:vault_comparison}. Несмотря на то, что удалось обезопасить снапшоты, быстродействие катастроически упало. Это может быть не так критично при не слишком требовательных задачах, ведь снапшоты не блокируют процессы Phantom на длительное время. Однако тогда при активной работе за время создания нового снимка будет изменено очень много страниц, которые придется писать в новый снимок, что в свою очередь приведет к еще большему увеличению времени создания снимка. Это также приведет к увеличению временных промежутков между вызовами snapper, из-за чего появляется риск потери данных за более протяженный периоб при сбое. С другой стороны, Дмитрий Завалишин описывал проблему, при которой между созданиями снапшотов большое количество памяти используется и теряет свою актуальность, приводя в пример обсчет графики. При использовании ОС в похожих задачах, когда требуется сравнительно большое время между снимками, а потеря данных за этот период не будут так критичны, можно попытаться в значительной степени смягчить последствия. Проерить это лучше всего практическим использованием, однако сейчас это невозможно ввиду неготовности системы.
...
В работе, определенно, есть ряд серьезных ограничений. Во-первых, tresor не предназначен для продакшина. Об этом говорит сам автор. Проблема в том, что это единственная библиотека , предназначенная для шифрования блочного уровня. А после ухода автора из команды, ее педдержка стала еще хуже. Во-вторых, тестирование производилось на небольших объемах пользовательского пространства. Нельзя с уверенностью сказать, что при кратном увелимчении объема система будет работать с сопоставимыми уровнями быстродействия и стабильности. В-третьих, тестирование проводилось только на виртуальной машине с заглушками вместо многих реально рабочих компонентов. Хоть теущий сценарий и дает понять приблизительные результаты, это все еще в значительной степени отличается от использования в реальном мире.
...
Очевиден недостаток быстродействия, о чем было уже множество раз сказано. Среди видимых путей решения наблюдается только внесение исправлений в библиотеку. Другой способ - пожертвовав в некоторой, пока неисследованной, степени безопасностью, реализовать свой более простой подход к шифрованию. Сделать это можно и на уровне файловой системы. В Genode есть порт openssl, поэтому не придется самостоятельно реализовывать криптографические алгоритмы, что было бы недопустимо.



\section{Future work}

Очевидно, в дальнейшем стоит поработать над улучшением скорости. Однако есть и другая, не рассмотренная ранее сторона. Стоит обратить внимание на то, что isomem тоже использует блочное устройство. Это часть персистентного хранилища, куда вытесняютс страницы из ОЗУ. При завершении работы в нем остается сножество артефактов, что позволяет почти полностью восстановить состояние машины, которое было достигнуто к момент выключения. Этого следует избегать. se_vault несложно переделать для предоставления блочной сессии вместо сессии файловой системы. Это позволит применить его для шифрования содержимого блочного устройства isomem. Вынос файловой системы наружу повысит гибкость решения. Для большей строгости можно, напротив, поместить snapper и/или isomem внутрь sandbox. Довольно простое в плане реализации решение. На ранних этапах адаптации такой вариант использовался и в нашей работе, однако от него отказались в угоду конфигурируемости. При крайней необходимости защитить данные можно и сейчас, используя vfs-плагин \textit{block}, но при текущем уровне быстродействия это нежелательно. Другим направлением может стать попытка объединить snapper и tresor. Tresor тоже хранит снапшоты и использует инкремент при записи. В этом плане их функционал схож, однако пользуются им компоненты по-разному.



### Conclusion

В ходе этой работы я занимался исследованием  способов защиты снапшотов PhantomOS. Для этих целей был адаптирован компонент, выполняющий шифрование снимков. Помимо этого были внесены правки в другие компоненты для корректной работы se\_vault.

В ходе исследования обозначенные задачи были выполнены. Цель была частично достигнута: безопасность снимов удалось обеспесить, но это в значительнй степени сказалось на производительности. Результатом стало появление специализированного компонента для хранения снапшотов.

В дальнейшем это должно помочь при исследовании безопасности персистентных операционных систем в общем и порта PhantomOS на Genode в частности. Подобного рода работы должны благоприятно сказаться на возможностях таких операционных систем и могут поспособствовать более быстрому их распространению.

В большей степени я удовлетворен работой, однако направление исследования себя не исчерпало. В своих будующих работах я сфокусируюсь на оптимизации получившегося компонента и расширении сферы его применения в контексте данной операционной системы.
...
В ходе данного проекта мы исследовали методы обеспечения безопасности снимков PhantomOS. С этой целью мы модифицировали компонент, отвечающий за шифрование снимков. Кроме того, мы внесли изменения в другие компоненты, чтобы обеспечить правильную работу se\_vault.

Поставленные в рамках исследования задачи были частично решены. Цель была частично достигнута: безопасность снимков была успешно обеспечена, однако это существенно повлияло на производительность.

В будущем это должно помочь в исследовании безопасности персистентных операционных систем в целом и порта PhantomOS на Genode в частности. Работа такого рода должна положительно повлиять на возможности таких операционных систем и может способствовать их более быстрому внедрению.

Мы в целом удовлетворены проделанной работой, но направление исследования еще не исчерпано. В дальнейшей работе мы сосредоточимся на оптимизации полученного компонента и расширении сферы его применения в контексте данной операционной системы.
