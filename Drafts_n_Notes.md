...
Стандарт NIST SP~800-111 \cite{NIST_SP800111} выделяет в качестве основной угрозы потерю или кражу устройства --- сценарий, при котором злоумышленник, получивший физический доступ, располагает широким набором способов прочитать хранящиеся данные. Помимо физической кражи, данные в покое уязвимы перед инсайдерскими угрозами, компрометацией резервных копий и остаточным раскрытием информации при утилизации носителей. 

Значимость этой угрозы существенно возрастает в контексте персистентных
операционных систем. Снапшот PhantomOS не эквивалентен обычному файлу на
диске: он содержит полное состояние PVM на момент создания снимка, включая
все персистентные объекты, пользовательские данные и потенциально чувствительное
состояние времени выполнения --- в частности, ключи сессий или учётные данные,
находившиеся в памяти в момент снапшота. Это напрямую соответствует известной
проблеме безопасности снапшотов виртуальных машин: скомпрометированный снапшот
представляет большую угрозу, чем образ диска, поскольку содержит активное
содержимое памяти наряду с персистентными данными \cite{VMSecurity2020}.
Таким образом, незашифрованный снапшот является единственной точкой отказа
с точки зрения конфиденциальности всего состояния системы: получив файл
снапшота, злоумышленник получает полную возможность воспроизвести работу
системы на произвольном устройстве.

Следует также отметить временно́е измерение этой угрозы. Даже если снапшот
никогда явно не экспортируется, он хранится на диске в промежутке между
созданием и следующей перезаписью --- окно, в течение которого любой процесс
или пользователь с доступом к блочному устройству может прочитать его
содержимое. В компонентной модели Genode это означает, что скомпрометированный
компонент, владеющий блочной сессией, способен бесшумно извлечь всё
персистентное состояние системы, не взаимодействуя с PVM напрямую.

\subsection{Подходы к защите данных в покое}

Защита хранимых данных может быть достигнута несколькими механизмами.
Разграничение доступа на уровне ОС --- права доступа к файлам, мандатное
управление доступом --- ограничивает круг субъектов, способных прочитать
файл, однако не обеспечивает защиты от злоумышленника, обходящего ОС
напрямую, например путём загрузки с внешнего носителя. Меры физической
безопасности снижают, но не исключают риск. Надёжное уничтожение данных
предотвращает утечку при списании носителей, но не защищает активное
хранилище. Шифрование остаётся единственным механизмом, делающим данные
неразборчивыми для злоумышленника, получившего прямой доступ к носителю,
что делает его основным подходом, рекомендованным NIST для данных в покое
\cite{NIST_SP800111}.

\subsubsection*{Уровни шифрования}

Шифрование хранилищ может применяться на нескольких уровнях стека хранения
данных, каждый из которых имеет свои компромиссы \cite{NIST_SP800111}.

\textit{Аппаратное шифрование} (Self-Encrypting Drives, TCG~Opal) выполняется
внутри контроллера накопителя, прозрачно для ОС, с пренебрежимо малыми
накладными расходами на CPU. Основной недостаток состоит в том, что управление
ключами делегируется прошивке производителя, которая исторически демонстрировала
уязвимости \cite{Stallings2011}. Кроме того, если ОС скомпрометирована при
смонтированном и расшифрованном накопителе, аппаратное шифрование не
обеспечивает никакой защиты.

\textit{Блочное шифрование} (dm-crypt/LUKS, BitLocker) работает ниже уровня
файловой системы, на уровне сырых блоков, и прозрачно для ОС и всех
приложений. Оно зашифровывает тома целиком и не раскрывает никакой
информации об отдельных файлах: их размерах, именах или структуре каталогов
\cite{NIST_SP800111}. Стандартным шифром является AES в режиме XTS (AES-XTS),
разработанном специально для блочных накопителей, где каждый сектор должен
шифроваться независимо без необходимости отдельно хранить вектор инициализации
\cite{Stallings2011}. Исследования производительности криптографических файловых
систем подтвердили практическую осуществимость шифрования целых разделов: такие
системы способны эффективно использовать буферный кеш для исключения
избыточных криптографических операций \cite{Wright2003CryptoFS}.

\textit{Шифрование на уровне файловой системы} (eCryptfs, fscrypt) работает
с отдельными файлами или каталогами выше блочного уровня. Оно допускает
гранулярное управление ключами --- разные ключи для разных файлов, --- однако
оставляет метаданные в открытом виде: имена файлов, структура каталогов,
размеры и метки времени доступа остаются незашифрованными. Такой подход
уместен при избирательной защите, но недостаточен в случаях, когда весь образ
хранилища должен рассматриваться как единая конфиденциальная единица.

\textit{Шифрование на уровне приложения} делегирует шифрование самому
приложению, обеспечивая максимальную гранулярность и возможность применять
политики ключей на уровне отдельных объектов, однако требует явного управления
ключами и не позволяет защитить данные программ, не рассчитанных на работу
с шифрованием.

\subsubsection*{Накладные расходы на производительность и аппаратное смягчение}

Шифрование создаёт вычислительные накладные расходы. Бенчмарки на NVMe-системах
полностью на флеш-памяти показывают, что программное блочное шифрование способно
создавать существенную нагрузку на CPU: в некоторых конфигурациях RAID из трёх
зашифрованных NVMe-накопителей достаточно для полного насыщения процессора, при
этом потери производительности при последовательной записи на вращающихся дисках
достигают 79\%, а при последовательном чтении --- 53\% \cite{SCS_LUKS_2023}.

Современные CPU во многом нивелируют эту проблему через набор инструкций
AES-NI, реализующий раунды AES в специализированных аппаратных блоках.
Шифрование AES-XTS с аппаратным ускорением AES-NI позволяет защищать данные
NVMe-накопителей без существенного влияния на скорость чтения и записи
\cite{AESNI2020}. Разрыв в производительности между аппаратно-ускоренной и
программной реализацией AES значителен: AES-NI обеспечивает ускорение до
13{,}5~раз по сравнению с программным AES при одновременном снижении
энергопотребления примерно на 90\% \cite{AESNI2020}.

Для частного случая виртуальных дисков и снапшотов с версионированием
возникает дополнительная сложность, связанная с хранением метаданных
целостности на уровне секторов. Добавление информации на уровне секторов
для поддержки случайных векторов инициализации и механизмов контроля
целостности создаёт не более 22\% накладных расходов на запись и 3\% на
чтение, что считается приемлемым с учётом предоставляемых гарантий
безопасности \cite{RethinkingBlockEncryption2022}.

\subsubsection*{Блочное шифрование в Genode: библиотека Tresor}

В рамках фреймворка Genode блочное шифрование реализовано через библиотеку
Tresor, лежащую в основе компонента File Vault, представленного в Genode
21.05 \cite{Genode2105}. File Vault шифрует мастер-ключ с использованием
алгоритма AES Key Wrap (RFC~3394) с 256-битным SHA-хешем пользовательской
парольной фразы, а блочные данные сопровождаются деревом хешей, листья
которого представляют собой SHA-256 хеши отдельных блоков по 4~КБ.
Корневой хеш хранится в Trust Anchor и используется для аутентификации
состояния хранилища при каждом обращении \cite{Stein2021FileVault}. Это
обеспечивает одновременно конфиденциальность и верификацию целостности.
Хранилище применяет копирование при записи как для полезных данных, так
и для метаданных, гарантируя сохранение последнего полного консистентного
состояния вплоть до фиксации нового \cite{Stein2021FileVault}. Текущий
приоритет в разработке библиотеки Tresor --- корректность и надёжность
слоя блочного шифрования; вопросы производительности, такие как
параллелизация независимых запросов и распределение операций шифрования
по нескольким ядрам CPU, пока не решены \cite{Genode_FileVault_Perf}.

\subsubsection*{Выбор подхода}

Для защиты снапшотов PhantomOS блочное шифрование посредством Tresor
является естественным выбором по ряду причин. Снапшоты хранятся как
непрозрачные файлы на блочном устройстве, управляемом Snapper, --- их
внутренняя структура невидима для любых механизмов файлового уровня,
а сами они представляют всё состояние PVM без разделения на отдельные типы
данных. Шифрование на уровне файловой системы неприменимо, поскольку
единицей хранения является сам файл снапшота. Шифрование на уровне
приложения потребовало бы модификации внутренней логики PVM или Snapper.
Аппаратное шифрование не обеспечивает защиты в рамках компонентной
архитектуры Genode, где в роли злоумышленника может выступать другой
компонент, владеющий блочной сессией. Блочное шифрование через Tresor
перехватывает данные именно на нужном уровне: ниже Snapper, выше физического
блочного устройства, прозрачно для всех вышестоящих уровней.
